...
//
// A route with multiple dialogs (invite / edit / revoke, …) drives a single
// open dialog from a `?dialog=<name>` search param. The boilerplate — open via
// `navigate({ search })`, close while clearing the entity id(s), derive which
// dialog is active — was hand-rolled per route and drifting (each
...
>({
  ...
  const open = useCallback(
    (dialog: TDialog, params?: Partial<TSearch>) =>
      // Functional updater preserves unrelated params.
